...
#This version adds a few more features, including flashing airports whose wind speed is greater than the set value. For example 15 kts.
#It also will simulate lightning at airports which are reporting Thunderstorms, Tornados, etc.
...
#Modify the ./NeoSectional/airports file with the airport codes to display on the map.
#Use "NULL" to turn off a bulb in a position where the LED won't be displayed.
#Use "LGND" in the position where you would like to use a bulb as a legend. 5 positions, 6 positions or 7 positions can be set aside for a legend.
#One note, the last airport listed in the "airport" file cannot be "NULL" otherwise the script will light all the LED's White.
#However, it can be "LGND" if you want to use the last pin as part of the legend.
...
# LED strip configuration YOU MUST CHANGE LED_COUNT VALUE:
LED_COUNT      = 70      #Number of LED pixels. Change this value to match the number of LED's being used on map
...
max_wind_speed = 13      #Any speed above will flash the LED for the appropriate airport
...
while (infinite):
	...
	while time.time() < timeout_start + (update_interval * 60): #take update interval which is in mins and turn into seconds
		...
		strip.show()
		time.sleep(blink_pause) #pause Blink before continue
